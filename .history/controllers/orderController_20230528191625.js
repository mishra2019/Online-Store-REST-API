...
dotenv.config();
// extracting the products from cart
...
export const addToCard = async (req, res) => {
  try {
    ...
    const prevOrderDetails = await Order.findOne({
      userName: req.user.userName,
      productName: req.body.productName,
    });
    let newOrder = null;
    if (prevOrderDetails) {
      orderDetails.productQuantity =
        Number(orderDetails.productQuantity) +
        Number(prevOrderDetails.productQuantity);
      if (product.quantity < orderDetails.productQuantity) {
        return res
          .status(404)
          .json({ error: "Too much quantity.. Unavailable" });
      }
...
// order plaed from the cart and update the given product quantity.
export const checkOutOneByOne = async (req, res) => {
  try {
    ...
    if (order.productQuantity > product.quantity) {
      return res
        .status(404)
        .json({ error: `${req.params.productName} is out of stock` });
    }
